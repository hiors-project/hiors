...
class RemapWrapper(gym.ObservationWrapper):
    ...
    def _tree_map(self, func, tree):
        """
        Apply a function to each leaf in a tree structure (dict, tuple, or single value).
        """
        if isinstance(tree, dict):
            return {k: self._tree_map(func, v) for k, v in tree.items()}
        elif isinstance(tree, tuple):
            return tuple(self._tree_map(func, v) for v in tree)
        else:
            return func(tree)

    def observation(self, observation):
        return self._tree_map(lambda x: observation[x], self.new_structure)
