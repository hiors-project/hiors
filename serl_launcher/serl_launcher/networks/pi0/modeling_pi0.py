...
policy = PI0Policy.from_pretrained("lerobot/pi0", revision="4329e3639a642c0dd3285e0cf42ba1031c61b5d5")
...
class PI0Policy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        ...
        if len(self._action_queue) == 0:
            ...
            actions = self.model.sample_actions(
                images, img_masks, lang_tokens, lang_masks, state, noise=noise, vis_attn=self.config.vis_attn
            )
    ...
    def forward_evaluate(self, batch: dict[str, Tensor], noise=None, time=None) -> dict[str, Tensor]:
        ...
        pred_actions = self.model.sample_actions(images, img_masks, lang_tokens, lang_masks, state, noise=None)
    ...
    def prepare_images(self, batch):
        ...
        for key in present_img_keys:
            ...
            # print(f"{key=}, {img.shape=}, {img.max()=}, {img.min()=}, {img.mean()=}")
    ...
    def prepare_language(self, batch) -> tuple[Tensor, Tensor]:
        ...
        # default prompt
        tasks = ["put the objects in the box" for i in range(len(batch["task"]))]
...
class PI0FlowMatching(nn.Module):
    ...
    def embed_suffix(self, state, noisy_actions, timestep):
        ...
        action_emb = self.action_in_proj(noisy_actions.to(dtype))  # torch.float32 -> bf16
    ...
    def sample_actions_with_logprob(self, images, img_masks, lang_tokens, lang_masks, state, noise=None, vis_attn=False, deterministic=False) -> tuple[Tensor, Tensor]:
        ...
        # Setup sigma schedule matching HuggingFace scheduler
        timesteps = torch.linspace(1.0, 0.0, num_steps + 1, device=device, dtype=torch.float32)
        sigmas = timesteps
        sigma_max = sigmas[1]

        # Start from noise
        x_t = noise
        total_log_prob = torch.zeros(bsize, device=device, dtype=torch.float32)
        ...
        for step_idx in range(num_steps):
            ...
            x_next = x_next_mean + noise_scale * variance_noise

            # No noise is added during evaluation
            if deterministic:
                x_next = x_t + dt * v_t

            # Compute log probability following reference formula
            log_prob = (
    ...
    def denoise_step(
        ...
        outputs_embeds, _, att_vis_output = self.paligemma_with_expert.forward(
